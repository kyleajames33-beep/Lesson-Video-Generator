...
export default function CircuitDiagram({ initialCircuit = "series", autoRotate = true, rotation, fixedCircuit, width = 720, height = 400, }) {
    ...
    const [rot, setRot] = useState({ yaw: -0.18, pitch: 0.16 });
    const [spinning, setSpinning] = useState(false); // gentle scenes: off by default
    const [selected, setSelected] = useState(null);
    const [hovered, setHovered] = useState(null); // component index
    ...
    const renderScene = () => {
        ...
        const P = (v) => project(v, effRot, cx, cy, scale);
        const items = [];
        ...
        circ.wires.forEach((wire, wi) => {
            ...
            items.push({
                ...
                el: (<g key={`w${wi}`}>
            <path d={d} fill="none" stroke={THEME.inkSoft} strokeWidth={3.5} strokeLinejoin="round" strokeLinecap="round"/>
            ...
            {!controlled && wire.flow && (<path d={d} fill="none" stroke={THEME.accentBright} strokeWidth={2.5} strokeDasharray="4 10" strokeLinecap="round" opacity={0.75} pointerEvents="none">
        ...
        circ.components.forEach((c, i) => {
            ...
            switch (c.kind) {
              ...
              <text x={-S * 0.5} y={-S * 0.55} fontFamily={THEME.font} fontWeight={800} fontSize={12} textAnchor="middle" fill={THEME.accentDeep}>+</text>
            </>);
                    plateW = S * 0.95;
                    break;
                case "resistor":
                    glyph = (<rect x={-S * 0.8} y={-S * 0.35} width={S * 1.6} height={S * 0.7} rx={3} fill={`url(#${uid}-metal)`} stroke={stroke} strokeWidth={2.5} filter={`url(#${uid}-shadow)`}/>);
                    plateW = S * 1.75;
                    plateH = S * 0.9;
                    break;
                case "switch":
              ...
              <circle cx={S * 0.65} cy={0} r={3.5} fill={THEME.ink}/>
                ...
                case "voltmeter":
                    glyph = (<>
              <circle cx={0} cy={0} r={S * 0.72} fill={`url(#${uid}-meter)`} stroke={stroke} strokeWidth={2.5} filter={`url(#${uid}-shadow)`}/>
              {/* Counter-rotate the meter letter so it stays upright. */}
              <text x={0} y={0} transform={`rotate(${-angle})`} textAnchor="middle" dominantBaseline="central" fontFamily={THEME.font} fontWeight={800} fontSize={S * 0.75} fill={THEME.accentDeep}>
                {c.kind === "ammeter" ? "A" : "V"}
              </text>
            </>);
                    plateW = S * 1.55;
            ...
            items.push({
                ...
                el: (<g key={`c${i}`} onClick={controlled ? undefined : toggleSelect("component", i)} onMouseEnter={controlled ? undefined : () => setHovered(i)} onMouseLeave={controlled ? undefined : () => setHovered(null)} style={{ cursor: controlled ? undefined : "pointer" }}>
            <g transform={`translate(${p.x} ${p.y}) rotate(${angle})`}>
              ...
              {glyph}
              {isSel && (<rect x={-plateW / 2 - 4} y={-plateH / 2 - 4} width={plateW + 8} height={plateH + 8} rx={7} fill="none" stroke={THEME.accentBright} strokeWidth={2.5} filter={`url(#${uid}-glow)`}/>)}
            ...
            items.push({
                depth: p.depth + 0.03,
                el: (<text key={`t${i}`} x={p.x + nx * off} y={p.y + ny * off} textAnchor="middle" dominantBaseline="central" fontFamily={THEME.font} fontWeight={800} fontSize={13} fill={THEME.accentDeep} pointerEvents="none">
            {c.value ? `${c.label} · ${c.value}` : c.label}
          </text>),
            });
            // Callout + hover chip at reserved depths (identical convention).
            if (isSel)
                items.push({ depth: 99, el: callout(`cc${i}`, p.x, p.y - plateH / 2, PART_LABELS.component(c)) });
            if (hovered === i && !isSel) {
    ...
    return (<div style={styles.root}>
      ...
      {!fixedCircuit && (<div style={styles.controlsRow} role="tablist" aria-label="Circuit type">
          {CIRCUITS.map((c) => (<button key={c.id} role="tab" aria-selected={c.id === activeId} style={{ ...styles.btn, ...(c.id === activeId ? styles.btnActive : {}) }} onClick={() => setActiveId(c.id)}>
              {c.name}
            </button>))}
        </div>)}

      <svg viewBox={`0 0 ${width} ${height}`} style={styles.stage} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp} onClick={controlled ? undefined : onStageClick} aria-label={`${circ.name} diagram. Click a component to learn what it does.`}>
        <defs>
      ...
      <div style={styles.infoPanel}>
        {circ.readouts.map((r) => (<Info key={r.label} label={r.label} value={r.value}/>))}
      </div>
