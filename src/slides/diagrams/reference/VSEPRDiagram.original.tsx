...
/** One entry per shape shown in the geometry selector.
 *  `positions` = unit vectors for electron domains around the central
 *  atom. Bonding-pair directions come FIRST, lone-pair directions LAST
 *  (the renderer reads bondingPairs to split the array). */
interface GeometryEntry {
  id: string;
  /** Exact NSW syllabus / textbook shape name shown to students. */
  name: string;
  electronDomains: number;
  bondingPairs: number;
  lonePairs: number;
  /** Displayed verbatim, e.g. "109.5°" or "90° and 120°". */
  bondAngle: string;
  /** Example molecule used to render the scene. */
  example: { formula: string; central: string; outer: string };
  /** One-line explanation in syllabus language for the info panel. */
  note: string;
  /** Longer "why this shape?" explanation, shown when expanded. */
  detail: string;
  positions: Vec3[];
}
...
const TRI_PLANAR: Vec3[] = [0, 1, 2].map((i) => {
...
const GEOMETRIES: GeometryEntry[] = [
  {
    ...
    detail:
      "Beryllium has exactly two regions of electron density and no lone pairs. The two bonding pairs repel each other, and the furthest apart they can get is directly opposite each other — a straight line with a 180° bond angle.",
    positions: [
      { x: 1, y: 0, z: 0 },
      { x: -1, y: 0, z: 0 },
    ],
  },
  {
    ...
    detail:
      "Three bonding pairs and no lone pairs around boron maximise their separation by spreading into a flat triangle. Every F–B–F angle is exactly 120°, and all four atoms lie in the same plane.",
  ...
  },
  {
    ...
    detail:
      "Nitrogen's four electron domains arrange tetrahedrally, but one is a lone pair. It occupies more space than a bonding pair and pushes the three N–H bonds slightly closer together (about 107°). The atoms form a pyramid with a triangular base — trigonal pyramidal.",
  ...
  },
  {
    ...
    detail:
      "Four bonding pairs can spread further apart in three dimensions than in a flat square (which would give only 90°). Pointing at the corners of a tetrahedron gives every H–C–H angle 109.5° — the maximum possible separation for four domains.",
    positions: TETRA,
  },
  {
    ...
    detail:
      "Five domains cannot all be equivalent. Three equatorial bonds form a flat triangle at 120°, and two axial bonds point straight up and down at 90° to that plane. This is the first shape where two different bond angles appear in one molecule.",
  ...
  },
  {
    ...
    detail:
      "Six equivalent bonding pairs point along the positive and negative x, y and z axes — the corners of an octahedron. Every adjacent F–S–F angle is 90°, and all six positions are identical by symmetry.",
    positions: [
      { x: 1, y: 0, z: 0 },
      { x: -1, y: 0, z: 0 },
      { x: 0, y: 1, z: 0 },
...
const PART_LABELS = {
  bond: (g: GeometryEntry) => `${g.example.central}–${g.example.outer} bond — a shared pair of electrons`,
...
export interface VSEPRDiagramProps {
  /** Initial shape shown. One of the GEOMETRIES ids. */
  ...
  rotation?: Rotation;
  /** REMOTION HOOK: force a shape and hide the selector buttons. */
...
export default function VSEPRDiagram({
  ...
  const [rot, setRot] = useState<Rotation>({ yaw: -0.6, pitch: 0.35 });
  const [spinning, setSpinning] = useState(autoRotate && !rotation);
  const [selected, setSelected] = useState<Selection>(null);
  const [hovered, setHovered] = useState<number | null>(null); // -1 = central, 0..n = outer atom index
  ...
  const renderScene = () => {
    ...
    items.push({
      depth: 0,
      el: (
        <g key="central" style={{ cursor: controlled ? undefined : "pointer" }}
          onMouseEnter={controlled ? undefined : () => setHovered(-1)}
          onMouseLeave={controlled ? undefined : () => setHovered(null)}>
          <circle cx={cx} cy={cy} r={R_CENTRAL}
            fill={`url(#${uid}-g-${geo.example.central})`} filter={`url(#${uid}-shadow)`} />
          ...
          <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central"
            fontFamily={THEME.font} fontWeight={800} fontSize={R_CENTRAL * 0.85}
            fill={contrastText(ELEMENT_COLORS[geo.example.central])} pointerEvents="none">
            {geo.example.central}
          </text>
        </g>
      ),
    ...
    bondDirs.forEach((dir, i) => {
      ...
      items.push({
        ...
        el: (
          <g key={`b${i}`} onClick={controlled ? undefined : toggleSelect("bond", i)}
            style={{ cursor: controlled ? undefined : "pointer" }}>
            {/* invisible wide hit area so the bond is easy to click */}
            <line x1={cx} y1={cy} x2={bondEnd.x} y2={bondEnd.y}
              stroke="transparent" strokeWidth={18} strokeLinecap="round" />
            <line x1={cx} y1={cy} x2={bondEnd.x} y2={bondEnd.y}
              stroke={isSel ? THEME.accentBright : "#9fb4bc"} strokeWidth={isSel ? w + 2 : w}
              strokeLinecap="round" opacity={bondOpacity}
              filter={isSel ? `url(#${uid}-glow)` : undefined} />
            {/* ambient pulse along the bond — SMIL, interactive mode only */}
            {!controlled && !isSel && (
              <line x1={cx} y1={cy} x2={bondEnd.x} y2={bondEnd.y}
                stroke={THEME.accentBright} strokeWidth={w * 0.5}
                strokeLinecap="round" pointerEvents="none">
                <animate attributeName="opacity" values="0;0.35;0" dur="3s"
      ...
      items.push({
        depth: atomP.depth,
        el: (
          <g key={`a${i}`} opacity={depthLerp(atomP.depth, 0.6, 1)}
            onMouseEnter={controlled ? undefined : () => setHovered(i)}
            onMouseLeave={controlled ? undefined : () => setHovered(null)}>
            <circle cx={atomP.x} cy={atomP.y} r={r}
              fill={`url(#${uid}-g-${geo.example.outer})`} filter={`url(#${uid}-shadow)`} />
            <text x={atomP.x} y={atomP.y} textAnchor="middle" dominantBaseline="central"
              fontFamily={THEME.font} fontWeight={800} fontSize={r * 0.8}
              fill={contrastText(ELEMENT_COLORS[geo.example.outer])} pointerEvents="none">
              {geo.example.outer}
            </text>
          </g>
        ),
      ...
      if (isSel) items.push({ depth: 99, el: callout(`cb${i}`, (cx + atomP.x) / 2, (cy + atomP.y) / 2, PART_LABELS.bond(geo)) });
      if (hovered === i && !isSel) {
        items.push({
          depth: 98,
    ...
    loneDirs.forEach((dir, i) => {
      ...
      items.push({
        depth: tip.depth,
        el: (
          <g key={`lp${i}`} onClick={controlled ? undefined : toggleSelect("lone", i)}
            style={{ cursor: controlled ? undefined : "pointer" }}
            opacity={depthLerp(tip.depth, 0.7, 1)}>
            <g transform={`rotate(${angle} ${mid.x} ${mid.y})`}>
              <ellipse cx={mid.x} cy={mid.y} rx={Math.max(len / 2, 8)} ry={lobeW}
                fill={THEME.lonePair} opacity={isSel ? 0.55 : 0.28}
                stroke={isSel ? THEME.accentBright : "none"} strokeWidth={2.5}
                filter={isSel ? `url(#${uid}-glow)` : undefined} />
      ...
      if (isSel) items.push({ depth: 99, el: callout(`cl${i}`, tip.x, tip.y, PART_LABELS.lone()) });
    });
  ...
  return (
    <div style={styles.root}>
      ...
      {!fixedGeometry && (
        <div style={styles.controlsRow} role="tablist" aria-label="Molecular geometry">
          {GEOMETRIES.map((g) => (
            <button key={g.id} role="tab" aria-selected={g.id === activeId}
              style={{ ...styles.btn, ...(g.id === activeId ? styles.btnActive : {}) }}
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={styles.stage}
        ...
        aria-label={`3D model of ${geo.example.formula}, ${geo.name} shape. Click bonds and lone pairs to learn what they are.`}>
        <defs>
          {/* Sphere gradient recipe: specular hot-spot → light body →
              base colour → shaded rim. Gives a glossy game-like ball. */}
          {elementsUsed.map((sym) => (
            <radialGradient key={sym} id={`${uid}-g-${sym}`} cx="33%" cy="27%" r="80%">
